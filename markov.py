...
## INSERT YOUR CODE HERE #### INSERT YOUR CODE HERE #### INSERT YOUR CODE HERE ##
  return value
  ...
  while True:
    ...
    for s in range(mdp.stateMax+1):
      ...
      for a in mdp.applicableActions(s):
        if not mdp.possible(a, s):
          break
        actionSum = 0
        for s2 in mdp.successors(s,a):
          s2,p,r = s2
          actionSum = actionSum + p * (r + gamma*V[s2])

        actionValueList.append(actionSum)

      if len(actionValueList) == 0:
        V[s] = 0
      else:  
        V[s] = max(actionValueList)

    toPolicy = True
    for s in range(mdp.stateMax+1):
      if V[s]-Vprev[s]>(epsilon*(1-gamma)/(2*gamma)):
        toPolicy = False
...
## INSERT YOUR CODE HERE #### INSERT YOUR CODE HERE #### INSERT YOUR CODE HERE ##
  ...
  for a in mdp.applicableActions(s):
    ...
    actionValueDict[a] = actionSum

  bestValue = 0
  bestAction = 0
  for a in mdp.applicableActions(s):
    if actionValueDict[a] > bestValue:
      bestValue = actionValueDict[a]
      bestAction = a
